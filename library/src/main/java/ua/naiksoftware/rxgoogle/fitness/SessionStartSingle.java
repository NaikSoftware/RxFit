...
public class SessionStartSingle extends BaseSingle<Status> {
    ...
    public SessionStartSingle(RxGoogle rxFit, Session session, Long timeout, TimeUnit timeUnit) {
    ...
    protected void onGoogleApiClientReady(GoogleApiClient apiClient, final SingleSubscriber<? super Status> subscriber) {
        setupPendingResult(Fitness.SessionsApi.startSession(apiClient, session), new StatusResultCallBack(subscriber));
    }
}
